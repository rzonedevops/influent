define(['jquery', 'lib/module', 'lib/channels', 'lib/render/cardRenderer', 'lib/render/fileRenderer', 'lib/render/clusterRenderer', 'lib/util/xfUtil', 'lib/ui/xfLinkType'],
function($, modules, chan, cardRenderer, fileRenderer, clusterRenderer, xfUtil, xfLinkType){
    ...
    var _sankeyState = {
        ...
        minSankeyWidth : 1,
        maxSankeyWidth : 20,
        targetOffsetY : 5, // This value is to add just a little more curvature to otherwise straight links.
        capture : false,
    ...
    var _getCardMetrics = function(sourceObj){
        ...
        var isDescendantOfFile = xfUtil.isUITypeDescendant(sourceObj, 'xfFile');
        if (uiType == 'xfFile' || isDescendantOfFile){
            return {
        ...
        else if (uiType == 'xfImmutableCluster' || uiType == 'xfMutableCluster'){
    ...
    var _createLinkSpecs = function(positionMap, workspaceObject, linkMap){
        ...
        for (var linkId in workspaceLinks){
            if (workspaceLinks.hasOwnProperty(linkId)) {
                ...
                var sourceHeight = cardRenderer.getCardHeight(sourceObj.getVisualInfo().showDetails);

                var targetObj = linkObj.getDestination();
                var targetId = _getNormalizedXfId(targetObj);
                var targetHeight = cardRenderer.getCardHeight(targetObj.getVisualInfo().showDetails);
                ...
                if (sourceObj.getUIType() === 'xfFile' && sourceObj.getClusterUIObject() != null){
                    if (sourceObj.getClusterUIObject().isHighlighted()){
                        isHighlighted = true;
                    }
                }
                else if (targetObj.getUIType() === 'xfFile' && targetObj.getClusterUIObject() != null){
    ...
    var	_updateLinks = function( channel, data ) {
        ...
        // Match the Sankey canvas to the workspace canvas.
        var workspaceDiv = $('#workspace')[0];
        sankey.height(workspaceDiv.scrollHeight);
        sankey.width(workspaceDiv.scrollWidth);

        _sankeyState.capture ? sankey.show() : sankey.fadeIn(1000);
    };
    ...
    var _initializeModule = function() {
        ...
        linkLayer.map('sankey-anchor').from('sankey-anchor');

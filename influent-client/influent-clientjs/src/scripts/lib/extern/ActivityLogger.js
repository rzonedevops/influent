...
function activityLogger()
{
	...
	* disable the generation and transmission of logging messages by 
	* this library. 
	*/
	...
	*/
	...
	function logSystemActivity(actionDescription, softwareMetadata)
	{
		...
		if(!muteSystemActivityLogging)
		{
			...
			encodedSystemActivityMessage +="SYSACTION ";
			...
			sendHttpMsg(encodedSystemActivityMessage);
		}
	...
	*/
	...
	function logUILayout(actionDescription, uiElementName, visibility, leftBound, rightBound, topBound, bottomBound, softwareMetadata)
	{
		...
		{
			encodedSystemActivityMessage = writeHeader();
			encodedSystemActivityMessage +="UILAYOUT ";
			encodedSystemActivityMessage += writeVersionData();
			encodedSystemActivityMessage += writeUILayoutData(uiElementName, visibility, leftBound, rightBound, topBound, bottomBound);	
			encodedSystemActivityMessage += writeSWMetadata(softwareMetadata);
			encodedSystemActivityMessage += appendActionDescription(actionDescription);
	...
	*/
	...
	function sendHttpMsg(encodedLogMessage, placeInLine)
	{
		...
		}else if (placeInLine>ticketServed)
		{
			...
			httpConnection.send(encodedLogMessage);
		}
